...
params_est = [m1_est, m2_est, lc1_est, lc2_est, l1_est, l2_est, I1_est, I2_est, error_m];


for i=1:n_ind
    individuos{i} = ind_rand();
    %individuos{i} = rand(1,3)*3-1;
...
for it = 1: n_it
    it
    for i=1:n_ind
        fitness(i) = f_PD(f_eval_ind(individuos{i}), p_ini, params, params_est);
    end
    [~,pos] = sort(fitness);
