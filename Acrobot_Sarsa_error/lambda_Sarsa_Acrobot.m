...
%     Q_matrix{tabla} = (rand(length(Q_tabla{tabla,1}),length(Q_tabla{tabla,2}),length(Q_tabla{tabla,3}),length(Q_tabla{tabla,4}),length(actionList))*2-1)*10;
    e{tabla} = zeros(length(Q_tabla{tabla,1}),length(Q_tabla{tabla,2}),length(Q_tabla{tabla,3}),length(Q_tabla{tabla,4}),length(actionList));
end
...
for it=1:MaxIt
    ...
    t=[]; q1_t = []; q2_t = []; dq1_t = []; dq2_t = []; F_t=[]; n_pasos = 0;
    moves = 1;
    ...
    for i = 0:dt*4:t_total
        ...
        q1_m = mod(q1 + error_m * (rand-0.5) * 2*pi ,2*pi);
        q2_m = mod(q2 + error_m * (rand-0.5) * 2*pi ,2*pi);
        dq1_m = max(-4*pi, min( 4*pi, dq1 + error_m * (rand-0.5) * 8*pi));
        dq2_m = max(-9*pi, min( 9*pi, dq2 + error_m * (rand-0.5) * 18*pi));
        
        F_ant = F;
        w_a = w;
        w = zeros(1,length(actionList));
        for tabla = 1:48
            I(tabla) = find( q1_m >= Q_tabla{tabla,1}, 1, 'last' );
            J(tabla) = find( q2_m >= Q_tabla{tabla,2}, 1, 'last' );
            K(tabla) = find( dq1_m >= Q_tabla{tabla,3}, 1, 'last' );
            L(tabla) = find( dq2_m >= Q_tabla{tabla,4}, 1, 'last' );
            w = w + reshape(Q_matrix{tabla}(I(tabla),J(tabla),K(tabla),L(tabla),:),1,length(actionList));
        end
        ...
        if first_move == 0
            if flag == 0
                ...
                delta = reward + gamma.*w(pos) - w_a(pos_a);
                
                for tabla = 1:48
                    e{tabla}(I_a(tabla),J_a(tabla),K_a(tabla),L_a(tabla),pos_a) = ...
                        e{tabla}(I_a(tabla),J_a(tabla),K_a(tabla),L_a(tabla),pos_a) + 1;
                             
                    Q_matrix{tabla} = Q_matrix{tabla} + alpha * delta * e{tabla};
                    e{tabla} = e{tabla} * lambda * gamma;
